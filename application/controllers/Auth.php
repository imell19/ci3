...
class Auth extends CI_Controller
{
    ...
    {   
        ...
        if($this->form_validation->run() == false) {
            ...
            Congratulation,your account has been creted.
    ...
    private function _sendEmail($token, $type)
    {
        ...
        $config['smtp_user'] = 'imeliana819@gmail.com';//isi dengan email
        $config['smtp_pass'] = 'pimeemstvzmqnvre';
        $config['smtp_port'] = 465;
        $config['mailtype'] = 'html';
        $config['charset'] = 'utf-8';
        $this->email->initialize($config);
        $this->email->set_newline("\r\n");
        $this->email->from('imeliana819@gmail.com', 'web Programing SMKN8 Jember');
        $this->email->to($this->input->post('email'));

    if ($type == 'verify') {
        $this->email->subject('Account Verivication');
        $this->email->message('Clic this link to verify you account  :
        ...
        $this->email->message('Clic this link to reset your password :
...
public function verify()
{
    ...
    if ($user) {
        $user_token = $this->db->get_where('user-token',
        ...
        if ($user_token) {
            if (time() - $user_token['date_created'] <(60 * 60 * 24)) {
                ...
                $this->db->update('user');
                $this->db->set('user-token' , ['email' => $email]);
                $this->session->set_flashdata('message',
                '<div class=" alert alert-success" role="alert">' .
            ...
            $this->sesion->set_flashdata('message',
            '<div class=" alert alert-danger" role="alert">
       ...
       public function forgotPassword()
       {
        ...
        if ($this->form_validation->run() ==false) {
           ...
           if ($user) {
               ...
               $this->session->set_flashdata('message',
              '<div class=" alert alert-success" role="alert"> Please check your email to reset password!</div>');
              redirect('auth/forgotPassword');
            } else {
                $this->session->set_flashdata('message',
                '<div class=" alert alert-success" role="alert">Email is not registered or activated!</div>');
                redirect('auth/forgotPassword');
            }
        }
    }
